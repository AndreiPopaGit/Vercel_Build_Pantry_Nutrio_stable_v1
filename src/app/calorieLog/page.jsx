...
import LogFoodModal from '../../components/dailyLogFood/modals/LogFood';
import { TrashIcon, EditIcon } from '../../components/consumption/SVGIcons';
import { MEALS } from '../../constant/categories'; // Import MEALS for grouping

// --- Reusable Components (Keep CalorieCircle and MacroBar as they are) ---
...
const MealLog = ({ meal, totalCalories, items, onEdit, onDelete }) => (
    <div className="relative pl-8">
        ...
        <div className="ml-4">
            {/* Display meal based on UI grouping, not direct DB field */}
                ...
                {items.map(item => (
                    <div key={item.id} className="group flex justify-between items-center bg-gray-50 hover:bg-gray-100 p-3 rounded-lg transition-colors">
                        <div>
                            ...
                            {/* Display quantity and unit from DB */}
...
export default function CalorieLog() {
    ...
    const handleOpenModal = (meal) => {
        setEditingEntry(null); // Ensure we're adding new
    ...
    const handleEditEntry = (entry) => {
        ...
        setSelectedMealForModal(entry.meal || 'Breakfast'); // Pass meal if available, else default
        setModalOpen(true);
    };

    const handleSaveAndClose = (data) => {
        // The hook now handles both create and update
    ...
    // Group logs by meal (using the MEALS constant for structure)
    const groupedLogs = useMemo(() => {
        const groups = {};
        MEALS.forEach(meal => groups[meal] = []); // Initialize structure
        if (logEntries) {
            logEntries.forEach(entry => {
                // If your DB entries don't have a 'meal' field, you'll need logic
                // here to group them (e.g., based on created_at time)
                // For now, assuming entry object *might* have a 'meal' property for UI.
                const mealKey = entry.meal || 'Snacks'; // Default to Snacks if no meal specified
                if (groups[mealKey]) {
        ...
        <div className="bg-white h-full overflow-y-auto p-8 font-sans">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="relative border-l-2 border-gray-200 pl-8 space-y-10">
                     {MEALS.map((meal) => ( // Use MEALS constant to ensure order
                        <MealLog 
                            key={meal} 
                            meal={meal}
                            items={groupedLogs[meal] || []} // Use grouped data
                ...
                {/* --- Log Food Button --- */}
                <div className="mt-12">
                    {/* Opens modal defaulting to Breakfast, could be changed */}
            ...
            {/* Render the LogFoodModal */}
            <LogFoodModal 
                isOpen={isModalOpen}
                onClose={() => setModalOpen(false)}
                onSave={handleSaveAndClose}
                meal={selectedMealForModal} // Pass selected meal for modal title
